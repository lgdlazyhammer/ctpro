	...
	public static void generateOrderDetail(GeneOrderEntity geneOrderEntity){
		...
		for(int i=0; i<geneTaskEntityList.size(); i++){
			...
			for(int j=0; j<geneExecutionEntityList.size(); j++){
				GeneExecutionEntity geneExecutionEntity = geneExecutionEntityList.get(j);
				// 获取行为
				GeneActionEntity geneActionEntity = geneExecutionEntity.getGeneActionEntity();
				// 增加行为
				String actionId = CtproCoreServiceManager.getInstance().addAction(geneActionEntity.getCtproAction());

				geneExecutionEntity.getCtproExecution().setExecutionAction(actionId);
				// 增加执行
				String executionId = CtproCoreServiceManager.getInstance().addExecution(geneExecutionEntity.getCtproExecution());
			...
			// 增加命令
			String taskId = CtproCoreServiceManager.getInstance().addTask(geneTaskEntity.getCtproTask());
			taskList.add(taskId);
		}
		...
		// 增加命令
		CtproCoreServiceManager.getInstance().addOrder(geneOrderEntity.getCtproOrder());
	}
}
